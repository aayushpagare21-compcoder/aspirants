...
export async function mergeImagesVertically(imageBuffers: Buffer[]): Promise<Buffer> {
    try {
        // Get metadata of all images to calculate the final image height
        const metadataPromises = imageBuffers.map(buffer => sharp(buffer).metadata());
        const metadata = await Promise.all(metadataPromises);
        // Calculate the total height and max width of the final image
        const totalHeight = metadata.reduce((sum, { height }) => sum + (height || 0), 0);
